...
// ==========================================
// 🛎️ UPLOAD DE VENDAS (PDV) - APENAS PRATOS E ESTOQUE (SIMPLIFICADO)
// ==========================================
app.post('/api/weeks/upload', upload.single('file'), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // O ficheiro já vem decodificado do Make, então passamos direto para o leitor de HTML
    const $ = cheerio.load(fileContent);

    // 1. PEGAR A DATA REAL DO E-MAIL (Ex: 12/05/2026)
    let finalDate = "";
    const bodyText = $('body').text();
    // Procura qualquer data no formato DD/MM/YYYY no texto
    const dateMatch = bodyText.match(/(\d{2})\/(\d{2})\/(\d{4})/);
    
    if (dateMatch) {
        // Converte DD/MM/YYYY para YYYY-MM-DD para a Base de Dados
        finalDate = `${dateMatch[3]}-${dateMatch[2]}-${dateMatch[1]}`;
    } else {
        // Fallback para o dia de hoje. NUNCA mais usa o nome do arquivo (ex: W19)
        finalDate = new Date().toISOString().split('T')[0];
    }

    // 2. EXTRAÇÃO DOS PRATOS (Buscando a tabela "Produtos Vendidos")
    const pratosVendidos = new Map<string, number>();

    // Passa por todas as tabelas e foca apenas nas que chamam "PRODUTOS VENDIDOS"
    $('table').each((_i: number, table: any) => {
        const firstRowText = $(table).find('tr').first().text().toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
        
        if (firstRowText.includes('PRODUTOS VENDIDOS')) {
            $(table).find('tr').each((_j: number, tr: any) => {
                const tds = $(tr).find('td');
                if (tds.length >= 2) {
                    const nome = $(tds[0]).text().trim().toUpperCase();
                    
                    // Ignora cabeçalhos, rodapés e a palavra TOTAL
                    if (!nome || nome === 'PRODUTO' || nome === 'TOTAL' || nome.includes('SISTEMA AGILEPDV')) return;
    ...
    if (pratosVendidos.size === 0) {
        throw new Error("Não foi possível encontrar nenhum prato na tabela 'Produtos Vendidos'.");
    }

    // 3. CRUZAMENTO COM AS FICHAS TÉCNICAS (ABATIMENTO DE ESTOQUE)
    ...
    for (const [prato, qtd] of pratosVendidos.entries()) {
        totalPortions += qtd;
        const recipe = recipes?.find((r: any) => r.name.toUpperCase() === prato);
    ...
    const { data: weekRecord, error: weekErr } = await supabase.from('weeks').upsert({ 
        week_code: finalDate, // Usa a data real (ex: 2026-05-12)
    ...
    // Limpa registros de consumo antigos do mesmo dia para não duplicar abatimentos
    ...
    res.json({ success: true, message: `✅ Movimento de ${finalDate} salvo!\nPratos Lançados: ${totalPortions}` });
